...
function App() {
  ...
  useEffect(() => { saveState({ players, teamCount, mode, playersPerTeam, teamNames, matchName, matchDate }) }, [players, teamCount, mode, playersPerTeam, teamNames, matchName, matchDate])
  ...
  const copyTeams = async () => {
    if (!result) return
    const text = ['FOOTBALL MATCH TEAMS', matchName ? matchName.toUpperCase() : '', matchDate ? new Date(`${matchDate}T00:00:00`).toLocaleDateString() : '', '', ...result.teams.flatMap((team) => [`${team.name} — ${team.players.length} players`, ...team.players.map((player, index) => `${index + 1}. ${player.name}`), '']), ...(result.unassigned.length ? ['UNASSIGNED', ...result.unassigned.map((player) => `• ${player.name}`)] : [])].join('\n')
  ...
  const reset = () => { if (!window.confirm('Start a new match and clear this roster?')) return; clearState(); setPlayers([]); setResult(null); setTeamNames(['', '']); setMatchName(''); setMatchDate(''); setMessage('New match started.') }
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {result && <section className="results" aria-live="polite"><div className="results-heading"><div><div className="eyebrow">MATCH TEAMS / GENERATED</div><h2>{matchName || 'Your teams are ready'}</h2>{matchDate && <p className="result-date">{new Date(`${matchDate}T00:00:00`).toLocaleDateString(undefined, { dateStyle: 'full' })}</p>}</div><div className="result-actions"><button onClick={copyTeams}>{copied ? 'Copied!' : 'Copy teams'}</button><button onClick={shareTeams}>Share teams</button><button onClick={() => window.print()}>Print</button></div></div><div className="summary-bar"><span><strong>{players.length}</strong> players</span><span><strong>{teamCount}</strong> teams</span><span><strong>{mode === 'auto' ? distribution : `${teamCount * playersPerTeam} assigned`}</strong> distribution</span><span className="assigned-status">● {result.unassigned.length ? `${result.unassigned.length} unassigned` : 'All players assigned'}</span></div><div className="team-grid">{result.teams.map((team, index) => <article className={`team-card team-${index % 4}`} key={team.id}><div className="team-card-heading"><span className="team-index">0{index + 1}</span><div><h3>{team.name}</h3><span>{team.players.length} players</span></div></div><ol>{team.players.map((player) => <li key={player.id}>{player.name}</li>)}</ol></article>)}</div>{result.unassigned.length > 0 && <div className="unassigned"><strong>Unassigned</strong>{result.unassigned.map((player) => <span key={player.id}>{player.name}</span>)}</div>}</section>}
