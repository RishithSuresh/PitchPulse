...
function App() {
  ...
  useEffect(() => { saveState({ players, teamCount, mode, playersPerTeam, teamNames, matchName, matchDate, captainMode, captainId }) }, [players, teamCount, mode, playersPerTeam, teamNames, matchName, matchDate, captainMode, captainId])
  ...
  const randomize = () => {
    ...
    setIsShuffling(true); window.setTimeout(() => { try { setResult(generateTeams(players, teamCount, mode, playersPerTeam, teamNames)); if (captainMode === 'random') setCaptainId(players[Math.floor(Math.random() * players.length)].id); setMessage('Teams are ready.') } catch (error) { setMessage(error instanceof Error ? error.message : 'Unable to generate teams.') } finally { setIsShuffling(false) } }, window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 0 : 650)
  }
  ...
  const reset = () => { if (!window.confirm('Start a new match and clear this roster?')) return; clearState(); setPlayers([]); setResult(null); setTeamNames(['', '']); setMatchName(''); setMatchDate(''); setCaptainMode('none'); setCaptainId(''); setMessage('New match started.') }
